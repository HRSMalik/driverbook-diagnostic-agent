# db/fault_vehicles.py — staging collection for source documents that carry at least one DTC.
...
def ensure_fault_vehicles_collection(db) -> None:
    """Create the unique index on source_id. Idempotent."""
    db[_COLLECTION].create_index("source_id", unique=True)


def stage_fault_document(db, source_doc: dict[str, Any], extracted: dict[str, Any]) -> bool:
...
def mark_analyzed(db, source_id: str) -> None:
